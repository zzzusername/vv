			...
			<div class="zTable">
				<div class="elTable">
					<vue-scroll :ops="ops" ref="vs">
						<div class="scrollbox">
							<el-table ref="multipleTable" @selection-change="changeFun" :data="BoxStatus" tooltip-effect="dark" style="width: 100%">
				...
				<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-size="pagesize" :current-page.sync="currentPage" layout="total, prev, pager, next, jumper" :total="total"></el-pagination>
			</div>
		</div>
		<div id="menuAddModel">
			<el-dialog :close-on-click-modal="false" title="菜单 - 新增" :visible.sync="dialogTableVisibleadd">
				<el-form :model="add" ref="add" label-width="30%" class="demo-ruleForm">
					<div class="formTable">
						...
						<div class="block" id="activeCity_box">
							<el-form-item label="地区：" :rules="[{ required: true, message: ' '}]" prop="url">
								<div class="checkboxBg">
										<div v-html="addressHtml2" class="framework" @click="activeCityshow"></div>
									</div>
							...
							<ul id="example-1">
								<li v-for="item in terminalList" :key="item.index" @click="getprovince(item)">{{ item }}</li>
							</ul>
						...
						<el-tab-pane label="权限分配" name="first">
				    	<template>
					    ...
					    <el-tab-pane label="流媒体服务器" name="second">
							...
							<div id="medialist" >
									...
									<ul>
										<li @mousemove="loginglist" v-for="(item, index) in medialist" :key="index">{{item.name}}
									     <span class="closebox"  ><img src="../../assets/close.png" alt=""></span>
						...
						<el-tab-pane label="辅助服务器" name="third">
									<span class="SelectTitle">选择辅助服务器：</span>
										<div class="zSelect">
											<el-select v-model="value_Select" @change="getvalue" class="zgroup" placeholder="默认（本机">
												<el-option v-for="item in options" :key="item.index" :label="item.name" :value="item"></el-option>
											</el-select>
										</div>

						</el-tab-pane>
						<el-tab-pane label="监控资源分配" name="fourth">
								...
								<div class="scrolltree">
									<vue-scroll :ops="ops1" ref="vs">
									<el-tree :data="data" :props="props" :load="loadNode" node-key="id" lazy @check-change="handleCheckChange" @node-expand="nodeclick_paths" @node-click="handleNodeClick"></el-tree>
									</vue-scroll>
								</div>
						...
						<el-tab-pane label="终端通讯录分配" name="fifth">
							...
							<div id="mediatree">
								...
								<div class="scrolltree">
									<vue-scroll :ops="ops1" ref="vs">
									<el-tree :data="datareg" :props="props" :load="loadNode" node-key="id" lazy @check-change="handleCheckChange" @node-expand="nodeclick_reg" @node-click="handleNodeClick"></el-tree>
									</vue-scroll>
								</div>
