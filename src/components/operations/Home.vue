              ...
              <template>
                <el-tabs v-model="activeName" @tab-click="tabClick">
                  ...
                  <el-tab-pane :disabled="disabled" label="监控资源分配" name="fourth">
                    <div> 唐古拉地址 <span>清空</span></div>
                    <div id="mediatree" v-if="dialogtree_monitor">
                      <!-- 监控资源分配tree -->
                      <!--  :data="treeData" -->
                            <el-tree  ref="tree_monitor" :props="props1" @node-expand="nodeclick2" lazy  @node-click="handleNodeClick2"  :load="loadNode1"
         ...
         getresource_organizations(param)
            .then(function(res) {
             ...
             console.log(res.status)
                ...
                // this.$nextTick(() => {
                //   this.$refs.props_monitor.setCheckedNodes(treeDataArr);
                // });
                // resolve(treeData);
              }
            })
          ...
          _this.$http
            .post(
              ServerUrl +
              "/api/Provider/UserManagement/GetUserAuthorizationResourcesTree",
              param
            )
            .then(function(res) {
              _this.reLogin(res.data.code); //提示帐号登陆
              if (res.data.code == 0) {
                ...
                _this.$nextTick(() => {
                  _this.$refs.tree4List.setCheckedNodes(treeDataArr.concat(_this.$refs.tree4List.getCheckedNodes()));
                });
                resolve(myList);
              }
            })
